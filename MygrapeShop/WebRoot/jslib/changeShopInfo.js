...
function showOldShopNameTip(){
	
	//alert("test"+SUCCESS);
	$("#oldShopNameNews").html("");
	$("#oldShopNameTip").html("请输入原始店铺ID");
}
...
function checkOldInfo(){
	...
	var oldShopName=$("#oldShopName").val();
	//alert(userName);
	var oldShopPwd=$("#oldShopPwd").val();
	//alert(userPwd);
	if(oldShopName.length==0){
		
		$("#oldShopNameNews").html("请输入原始店铺ID");
		return;
	}
	if(oldShopName.length!=0){
		//alert(userName);
	...
	$.get("servlet/ShopHandleServlet?type=checkInfo&shopName="+oldShopName+"&shopPwd="+oldShopPwd,null,writeCheckInfo);
...
function checkinfo(){
	var result=$("#register");
	if(nameok==false||passwordok==false||passwordConfirmok==false){
